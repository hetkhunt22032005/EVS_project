...
<head>
    ...
    <title>Complete Today's Task</title>
   <style>
        body {
...
<body>
    <div class="container">
        ...
        <% 
            ...
            Integer taskId = null; // Initialize taskId variable outside try block 
            
            String url = "jdbc:postgresql://localhost:5432/EVS";
            String username = "postgres";
            String password = "admin";
            String sql = "SELECT taskid, task_date FROM daily_tasks";
            ...
            try {
                ...
                try (Connection conn = DriverManager.getConnection(url, username, password);
                    ...
                    // Iterate over the result set and check for today's task
                    while (rs.next()) {
                        String taskDate = rs.getString("task_date");
                        int currentTaskId = rs.getInt("taskid");
        ...
        <ul class="task-list">
            <% if (taskFound && taskId != null) { %>
                ...
                %>
            <% } else { %>
                <li class="no-task">No task scheduled for today.</li>
            <% } %>
        </ul>
        
        <%-- Form to submit the task completion --%>
        <form action="addpoints.jsp" method="post">
            <button type="submit" class="submit-button">I have Completed My Task Honestly</button>
        </form>
    </div>
</body>
</html>
